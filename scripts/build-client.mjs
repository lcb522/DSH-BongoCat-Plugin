// Build lib/client.js: embed the vendored runtimes and model files into the
// client template. Requires: vendor/ (three runtime libs) + assets/keyboard/.
...
// Model files: everything under assets/keyboard, keyed by repo-relative path.
const modelDir = join(root, 'assets', 'keyboard')
const modelFiles = {}
const walk = (dir) => {
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const full = join(dir, entry.name)
    if (entry.isDirectory()) walk(full)
    else modelFiles[relative(modelDir, full).replaceAll('\\', '/')] = full
  }
}
walk(modelDir)
...
replaceOnce('["__LIB_CORE__"][0]', JSON.stringify(core))
replaceOnce('["__LIB_PIXI__"][0]', JSON.stringify(pixi))
replaceOnce('["__LIB_L2D__"][0]', JSON.stringify(l2d))
replaceOnce('__MODEL_FILES__', JSON.stringify(MODEL_FILES))

writeFileSync(outPath, out)
const kb = (n) => `${Math.round(n / 1024)}KB`
console.log(`built lib/client.js: ${kb(out.length)} (core ${kb(core.length)} + pixi ${kb(pixi.length)} + l2d ${kb(l2d.length)} + model ${kb(out.length - core.length - pixi.length - l2d.length)})`)
